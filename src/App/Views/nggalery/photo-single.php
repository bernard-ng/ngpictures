...
if(isset($_SESSION['id']) and !empty($_SESSION['id']))
{
	...
	if(isset($_GET['type'],$_GET['id']) and !empty($_GET['type']) and !empty($_GET['id']))
	{
		...
		if($_GET['type'] == "user" )
		{
			$news=$base->prepare("SELECT * FROM galerie WHERE id=?");
			$news->execute(array($getID));

		}else if($_GET['type'] == "ngpic")
		{
		...
		if($news->rowcount() ==1)
		{
			...
			$tags= $news['tags'];
			$photoID= $news['id'];
			$posterID = photo_info($photoID,"posterID");
			$date= $news['date_pub'];
			...
			if(isset($_POST['cmtSubmit'])) 
			{
				...
				{  
					...
					$insert= $base->prepare("insert into commentaire (commentaire,photoID,userID,date_pub) values (?,?,?,now()) ");
			...
			<div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-offset-2 col-lg-offset-2 col-md-8 col-lg-8">
				<div class="row">
					<div class="card--date">
						...
						<img src="/galerie/images/640-640/<?= photo_min($photoID)?>"/>
		...
		<div class="row">	
			<div class=" ng-panel panel panel-primary ng-panel-active ">
		            ...
		            <li class="list-group-item  ">
						<a class="btn btn-primary btn-xs ng-btn" href="/php/script/download.php?type=photo&n=<?= photo_min($photoID)?>" role="button"><span class="glyphicon glyphicon-save"></span></a> Télécharger

					</li>
		            <li class="list-group-item  ">
						<a class="btn btn-primary btn-xs ng-btn" href="/php/script/like.php?type=photo&t=1&id=<?= $getID ?>" role="button"><span class="glyphicon glyphicon-thumbs-up"></span></a> <?= check_likep_statut($photoID,$_SESSION['id']) ?> 
						<span class="ng-badge badge"><?= KM_format(photo_info($photoID,"like")) ?></span> 
					</li>
					<li class="list-group-item "> 
						<a class="btn btn-primary btn-xs ng-btn" href="/php/script/like.php?type=photo&t=2&id=<?= $getID ?>" role="button"><span class="glyphicon glyphicon-thumbs-down"></span></a> <?= check_dislikep_statut($photoID,$_SESSION['id']) ?> 
						<span class="ng-badge badge"><?= KM_format(photo_info($photoID,"dislike")) ?></span> 	
							
					</li>
					<li class="list-group-item ">
						 <a class="btn btn-primary btn-xs ng-btn" href="/php/script/like.php?type=photo&t=3&id=<?= $getID ?>" role="button"><span class="glyphicon glyphicon-heart"></span></a> <?= check_lovep_statut($photoID,$_SESSION['id']) ?> <span class="ng-badge badge"><?= KM_format(photo_info($photoID,"love")) ?></span>    
						    
					</li>
					<li class="list-group-item ng-margin-default">	
                    <a  href="/galerie/mentions.php?type=photo&id=<?= $getID ?>" role="button"><span class="glyphicon glyphicon-chevron-down"></span> voir les mentions</a>
					</li>
	...
	<div class="row">
		<div class="ng-panel panel panel-primary">
			<div class="ng-panel panel-heading ng-margin-default" role="tab" >		                                     
		            ...
		            <span class="ng-badge badge"><?= KM_format(photo_info($photoID,"commentaire"))?></span>		                                
		    	</div>
		<?php 

			$commentaire= $base->prepare("SELECT * from commentaire where photoID=? order by date_pub desc limit 0,3");
				...
				<div class="media">
				 	<div class="media-left media-top">
				  	...
				  	<div class="media-body">
					    ...
					    <span class="pull-right">
					    <time>
					    	<span class="glyphicon glyphicon-time"></span> <?= temps($c['date_pub'])?></time></span>
				  	</div>
				</div>
			</li>
		<?php }?>

		<?php if(photo_info($photoID,"commentaire") > 3){?>

			<li class="list-group-item ng-margin-default">
				<a href="/galerie/comment.php?id=<?= $_GET['id']?>"><center>Voir plus</center></a>
			</li>
		</ul>
