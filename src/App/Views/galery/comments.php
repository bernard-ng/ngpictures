...
if(isset($_SESSION['id']) and !empty($_SESSION['id']))
{
			...
			if(isset($_GET['id']) and !empty($_GET['id']))
			{
				...
				if($news->rowcount() ==1)
				{
					...
					$commentaire= $base->prepare("SELECT * from commentaire where photoID=? order by date_pub desc ");
					$commentaire->execute(array($photoID));

					$comN = $base->prepare("SELECT * from commentaire where photoID=? order by date_pub desc ");
...
<div class="col-xs-12 col-sm-8 col-md-8 col-lg=8" >
	...
	<div class="ng-panel panel-heading ng-margin-default">COMMENTAIRE<?= $t = (photo_info($photoID,"commentaire") > 1)? "S" : ""  ?> <span class="ng-badge badge"><?= $com ?></span></div>
