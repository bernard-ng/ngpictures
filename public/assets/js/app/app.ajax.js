    ...
    if (postContainer) {
        try {
            ...
            for (let i = 0; i < posts.length; i++) {
                if (!posts[i].hasAttribute('data-comment-initialized', 'true')) {
                    posts[i].setAttribute('data-comment-initialized', 'true');
...
function likes(element) {
    ...
    if (postContainer) {
        let posts = postContainer.getElementsByTagName("article");
        for (let i = 0; i < posts.length; i++) {
            if (!posts[i].hasAttribute('data-likes-initialized', 'true')) {
                posts[i].setAttribute('data-likes-initialized', 'true');
                try {
    ...
    if (typeof saveBtn !== 'undefined') {
        [].slice.call(saveBtn).forEach((saveBtn) => {
            if (!saveBtn.hasAttribute('data-collection-initialized', 'true')) {
                saveBtn.setAttribute('data-collection-initialized', 'true');
    ...
    if (typeof followBtns !== 'undefined') {
        [].slice.call(followBtns).forEach(function (followBtn) {
            if (!followBtn.hasAttribute('data-following-initialized', 'true')) {
                followBtn.setAttribute('data-following-initialized', 'true');
...
function downloadFile(element) {
    ...
    if (typeof downloadBtn !== 'undefined') {
        for (let i = 0; i < downloadBtn.length; i++) {
           if (!downloadBtn[i].hasAttribute('data-download-initialized', 'true')) {
               downloadBtn[i].setAttribute('data-download-initialized', 'true');
